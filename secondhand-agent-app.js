...
async function openDetail(productId, { basic = false } = {}) {
  ...
  let final = {
    verdict: ({ recommend: '추천', info: '확인 필요', low: '기다리기', reject: '비추천' }[getProductVerdict(item).key] || '확인 필요'),
  ...
  const verdictMap = {
    ...
    '확인 필요': { key: 'info', label: '정보 부족' },
    '기다리기': { key: 'low', label: '기다리기' },
    '비추천': { key: 'reject', label: '비추천' }
  };
  const listingVerdict = getProductVerdict(item); const verdict = { key: listingVerdict.key, label: listingVerdict.label };
